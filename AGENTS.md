...
- For manuscript work, keep pending experiment work in `docs/active`; use confirmed results and placeholders in the manuscript, do not invent results or alter core technical content, do not state that experiments still need to run, strip all internal draft/process language (e.g., submission strategy, pending confirmation, smoke, post-fix), and write in direct computational-paper style (result-led phrasing; avoid meta-commentary such as "this paper argues," "central contribution," or defensive "not X but Y" unless necessary). When the user supplies numbered exact LaTeX edits, apply them verbatim in order without unsolicited style or wording changes. When compressing Methods, keep the payoff table, scripted-partners disclaimer, and core equations in main text; replace appendix duplicates with cross-references rather than repeating equations.
