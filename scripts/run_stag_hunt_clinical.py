"""Run Stag Hunt clinical sensitivity: alexithymia, borderline, depression + normal baseline."""
...
CLINICAL_CONFIGS = [
    ("alexithymia", "affect_aif/configs/stag_hunt_clinical_alexithymia.json", 9),
    ("borderline", "affect_aif/configs/stag_hunt_clinical_borderline.json", 10),
    ("depression", "affect_aif/configs/stag_hunt_clinical_depression.json", 11),
]

OUTPUT_DIR = Path("results/stag_hunt_clinical")
...
def run_baseline(replications: int, rounds: int) -> pd.DataFrame:
    """Run Stag Hunt default with C2 + C4 as normal-affect baseline."""
    print("=" * 60)
    print("Running Stag Hunt C2 + C4 baseline")
    print("=" * 60)

    config = ExperimentConfig.from_json("affect_aif/configs/stag_hunt_default.json")
    ...
    for condition in config.conditions:
        for rep in range(replications):
            ...
            recs = runner.run_replication(
                ...
                replication=rep,
                seed=seed,
                config_path="affect_aif/configs/stag_hunt_default.json",
                config_name="stag_hunt_default",
            )
            records.extend(recs)
...
def run_clinical(replications: int, rounds: int) -> dict[str, pd.DataFrame]:
    ...
    for name, config_path, cond in CLINICAL_CONFIGS:
        print(f"\n{'='*60}")
        print(f"Running Stag Hunt clinical: {name} (C{cond})")
        print(f"{'='*60}")
...
def analyze(baseline_df: pd.DataFrame, clinical_dfs: dict[str, pd.DataFrame]):
    """Compare clinical variants against normal C2."""
    print(f"\n{'='*60}")
    print("STAG HUNT CLINICAL SENSITIVITY ANALYSIS")
    print(f"{'='*60}")
    ...
    print(f"\nBaselines:")
    print(f"  C4 (no affect):      {c4_baseline.mean():.1f} +/- {c4_baseline.std():.1f}")
    print(f"  C2 (normal affect):  {c2_normal.mean():.1f} +/- {c2_normal.std():.1f}")
    ...
    for name, (cond, label) in clinical_conditions.items():
        ...
        if len(common) > 1:
            ...
            d = float((clin.loc[common] - c2_normal.loc[common]).mean() / (clin.loc[common] - c2_normal.loc[common]).std())
        else:
            t, p, d = 0.0, 1.0, 0.0
        ...
        if len(common_c4) > 1:
            ...
            d_c4 = float((clin.loc[common_c4] - clin_c4.loc[common_c4]).mean() / (clin.loc[common_c4] - clin_c4.loc[common_c4]).std())
        else:
        ...
        print(f"    vs C2 normal: diff={clin.mean() - c2_normal.mean():+.1f}, d={d:.2f}, p={p:.4f}")
        print(f"    vs own C4:    diff={clin.mean() - clin_c4.mean():+.1f}, d={d_c4:.2f}, p={p_c4:.4f}")

    # Between-clinical differentiation
    clinical_means = {r["variant"]: r["mean_payoff"] for r in rows}
    if len(clinical_means) == 3:
        vals = list(clinical_means.values())
        spread = max(vals) - min(vals)
        print(f"\n  Between-clinical spread: {spread:.1f} points")
        print(f"  (Compare: graded game spread was ~0.03 points)")

    results_df = pd.DataFrame(rows)
    results_df.to_csv(OUTPUT_DIR / "analysis.csv", index=False)
    print(f"\nSaved analysis to {OUTPUT_DIR / 'analysis.csv'}")
