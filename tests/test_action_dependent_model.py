...
from tasks.trust.pomdp import build_trust_pomdp_template, partner_action_distribution, stance_transition_for_action
...
def test_graded_model_uses_action_strength_for_stance_transitions():
    ...
    low = stance_transition_for_action(model, 0)
    high = stance_transition_for_action(model, 5)
    mid = stance_transition_for_action(model, 3)
...
def test_trust_model_predicts_partner_actions_from_type_and_stance_beliefs():
    ...
    prediction = partner_action_distribution(model, belief)
