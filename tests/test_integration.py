...
def test_batch_workers_one_runs_directly_without_process_task_wrapper(tmp_path, monkeypatch):
    ...
    def fail_process_task(*_args, **_kwargs):
        raise AssertionError("workers=1 should run directly without the process task wrapper")

    monkeypatch.setattr("experiments.trust.batch.run_variant_replication_task", fail_process_task)

    batch = BatchExperimentRunner(
        config_paths=[str(path)],
        output_root=str(tmp_path / "results"),
        batch_id="direct_inline_batch",
        workers=1,
        verbose=False,
    )
    result = batch.run_all()
...
def test_batch_inline_runner_resumes_completed_checkpoint_runs(tmp_path):
    path = write_example_toml(tmp_path / "betrayal_choice.toml", rounds=2, replications=2)
    path.write_text(path.read_text().replace("auto = true", "auto = false", 1), encoding="utf-8")
    runner = BatchExperimentRunner(
        config_paths=[str(path)],
...
def test_batch_checkpoint_resume_filters_without_row_apply(tmp_path, monkeypatch):
    path = write_example_toml(tmp_path / "betrayal_choice.toml", rounds=2, replications=1)
    runner = BatchExperimentRunner(
        config_paths=[str(path)],
        output_root=str(tmp_path / "results"),
    ...
    def fail_apply(*_args, **_kwargs):
        raise AssertionError("checkpoint resume should not use row-wise DataFrame.apply")

    monkeypatch.setattr(pd.DataFrame, "apply", fail_apply)

    completed = runner._load_checkpoint(state)

    assert (run.variant_id, run.seed, run.replication) in completed
    assert len(state.primary_rows) == 2
...
def test_serial_checkpoint_resume_filters_without_row_apply(tmp_path, tiny_spec, monkeypatch):
    ...
    assert (run.variant_id, run.seed, run.replication) not in executed
    assert len(results) == len(spec.expand_runs()) * 2
