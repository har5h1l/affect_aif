...
This appendix specifies the focal agent's partner-local generative model in standard discrete active-inference form. Each partner \(k\) has a POMDP \(\mathcal{M}_k\) with hidden type, stance, and own-investment bookkeeping. The model follows Appendix~\ref{app:generative_process} but is not identical to the environment: payoff is an observation modality marginalizing over partner response; scheduled betrayal and repair are environment-side; affective precision is external to the POMDP.
...
\end{align*}
\(s^{\mathrm{own}}\) indexes executed investment and is fully observable: deterministic bookkeeping that supplies the focal agent's own investment level to the payoff likelihood, not an inferred hidden variable. The joint hidden state per partner spans \(4\times3\times6=72\) configurations.
