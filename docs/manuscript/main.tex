...
% camera-ready build: use the public repository rather than the review mirror
\newif\ifanonsupprepo
\anonsupprepofalse
\newcommand{\anonsupprepourl}{the repository \texttt{README.md}}
\newcommand{\codereleaseartifactlist}{the simulation environments, precision conditions, experiment configuration files, analysis scripts, and figure-generation code used for the reported experiments}
...
Andrew Pashea\inst{2}}
\authorrunning{H. Shah and A. Pashea}
\institute{Mission San Jose High School
\and
...
\caption{Partner-local precision is more strongly associated with partner-response surprisal than realized payoff. Left and middle panels show absolute partial correlations; the right panel shows payoff. Error bars show 95\% CIs.}
...
\caption{Tracked-only preserves $\bar{\beta}_k$ movement without the policy-sharpening effect. Full deployment lowers policy entropy; payoff is shown for context. Error bars show 95\% CIs.}
...
All reported confidence intervals are two-sided 95\% percentile bootstrap intervals based on 10,000 resamples with bootstrap seed 0. Matched variants are compared by resampling seed-level paired differences. Partial-correlation intervals resample complete simulation-seed clusters, and time-course intervals resample seed-level means within each round bin.
