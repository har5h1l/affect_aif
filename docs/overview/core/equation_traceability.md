...
| Eq. 2 | `eq:charge` | `tasks.trust.affect.DiscreteBetaState.update(...)` calls private helper `_affective_charge(...)`; defaults come from `LOG_SURPRISE_BASELINE_SQ` and variant/config fields `alpha_charge`, `sigma_0_sq`, and `charge_transform`. `affective_charge_variants(...)` records both formula values, while `affective_charge_active` records the value actually used by the beta update. | The default active update is `phi = alpha * (sigma_0_sq - epsilon^2)`. The explicit `linear` transform converts the legacy stored `sigma_0_sq` once to `sigma_0`, giving `phi = alpha * (sigma_0 - epsilon)`. Both share the same neutral surprise. | `tests/test_trust_affect.py::test_log_surprise_baseline_matches_fifty_fifty_prediction`; `tests/test_trust_affect.py::test_linear_shadow_charge_has_the_same_neutral_surprise_as_equation_two`; `tests/test_trust_affect.py::test_linear_charge_updates_beta_from_the_linear_formula`; `tests/test_trust_affect.py::test_low_surprise_decreases_beta_rate`; `tests/test_trust_affect.py::test_high_surprise_increases_beta_rate` |
