...
- **F (multi-focal-agent runtime — design + plan only)** — 2026-04-18: design spec at `docs/superpowers/specs/2026-04-18-multi-focal-runtime-design.md`, implementation plan at `docs/superpowers/plans/2026-04-18-multi-focal-runtime-plan.md`. Strict pomdp_spec §12 turn-taking (single focal per round); RoundProtocol extension seam reserves room for future all-pairs. Simultaneous-moves resolution (deviation from §12 step 4) documented as decision F4 — required to respect post-B+A decision #6. Heterogeneous AIF populations via `agents: [...]` config schema. Four new configs land for sub-project D's inventory: `multifocal_smoke.json`, `multifocal_homogeneous_affective.json`, `multifocal_clinical_mix.json`, `multifocal_assortative_choice.json`. Implementation deferred to a fresh executing-plans session per the superpowers handoff rule.
