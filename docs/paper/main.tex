...
\paragraph{Results.} Both policies achieve non-zero reward on the CvC benchmark (Table~\ref{tab:cvc}), confirming the architecture's viability in spatial settings. BFS pathfinding with aoe\_mask-based wall detection achieves 84--91\% movement success. The beta-tracking mechanism correctly identifies all 7 teammates via their \texttt{agent\_id} observation feature and maintains per-partner precision estimates.

\begin{table}[ht]
\centering
\caption{CvC benchmark results (machina\_1, 1000 steps, 5 seeds). Reward is per-agent mean.}
\label{tab:cvc}
\begin{tabular}{lccc}
\toprule
Policy & Mean Reward & Aligned Junctions & Move Success \\
\midrule
ScoringLoopPolicy & 0.092 & 2.0 & 84.2\% \\
AffectCvCPolicy   & 0.092 & 2.0 & 84.2\% \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Interpretation.} In homogeneous teams (all agents run the same policy), $\bar{\beta}$ stabilizes at $\sim$0.65---correctly reflecting high teammate reliability. With team precision consistently above the cooperation threshold, the affect mechanism does not modulate behavior differently from the baseline. This is the \emph{correct} adaptive response: when all partners are reliably predictable, the precision signal should not perturb the coordination strategy. Behavioral differentiation requires heterogeneous teams or partner volatility, paralleling the trust-game finding that affect augmentation is most pronounced under betrayal stress. The spatial domain also dampens beta dynamics relative to the trust game: positional prediction errors are normalized over a larger observation range (Manhattan distance up to 26 vs.\ binary cooperation), and random spatial movement has lower surprise than random cooperation decisions.
