...
The discrete variational formulation casts $\beta_k$ as a discrete hidden state with $L = 5$ precision levels, explicit likelihood $P(\epsilon \mid \beta) \propto \exp(\alpha(\sigma_0^2 - \epsilon^2) \cdot \beta)$, and tridiagonal persistent transition dynamics. The posterior update uses the same categorical Bayesian machinery as partner-type inference:
\begin{equation}
...
The central result: under sophisticated inference, explicit planning depth is irrelevant in the binary trust game, but affect provides consistent augmentation (Table~\ref{tab:horizon}).
...
The betrayal result (Table~\ref{tab:bms}) is particularly strong: precision tracking produces the best \emph{predictive} model of partner behavior under volatility, not just higher payoffs. The $\log_{10}$ BF of 3.0 against C1 is far above the ``decisive'' threshold of 2.0 \citep{kass1995bayes}.
...
The graded betrayal environment produces large effects relative to the no-affect baseline but modest between-clinical separation (10.324--10.353 payoff range). The Stag Hunt betrayal environment resolves this (Table~\ref{tab:clinical_sh}): its severe miscoordination cost (sucker payoff) amplifies the behavioral consequences of precision volatility, producing the first statistically significant between-clinical differentiation.
...
\bottomrule
\end{tabular}
\caption{Clinical sensitivity in Stag Hunt betrayal (50 seeds $\times$ 120 rounds). $d = (\text{phenotype} - \text{healthy})/\text{SD}$; positive $d$ = phenotype outperforms healthy. Borderline is the first phenotype with decisive Bayesian evidence of impairment relative to healthy.}
