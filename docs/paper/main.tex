...
% ── Abstract ──────────────────────────────────────────────────────────────────
\begin{abstract}
We present a computational model of affect as per-partner metacognitive precision tracking in multi-agent active inference. In the implemented trust-game experiments, the agent primarily infers partner type while the current interaction context (which partner is active) is task-given and observed. Each social partner is also assigned a slow-timescale affective state $\beta_k$ that summarizes the running reliability of the agent's predictive model for that partner. In the core pipeline, $\beta_k$ is an auxiliary affective precision-like state used to reweight policy evaluation, rather than a fully symmetric hidden-state factor throughout the main generative model. In a volatile multi-partner trust game, we show that: (1)~affect provides \emph{orthogonal} augmentation beyond the tested non-affective depth manipulations---within the current binary task, planners from horizon $\tau = 2$ to $\tau = 8$ are statistically indistinguishable, while adding the affective signal yields $d = 0.64$ improvement at every depth; (2)~ablating affect reproduces the somatic marker deficit---intact partner knowledge with impaired decision deployment; (3)~precision tracking and reward averaging occupy distinct, task-dependent informational niches, with precision tracking excelling under binary betrayal stress ($d = 0.59$) and reward averaging prevailing in continuous-investment settings ($d = 0.43$--$0.89$ over precision tracking); (4)~cumulative predictive log-score comparisons favor the affective model under volatility, and RFX-BMS over per-seed predictive scores assigns high exceedance probability to the affective agent in betrayal; (5)~augmentation generalizes across Prisoner's Dilemma, Stag Hunt, and Chicken games under betrayal ($d > 1.0$ in all three); and (6)~clinical parameter perturbations (alexithymia, borderline, depression) produce qualitatively distinct behavioral phenotypes when the environment makes affective dynamics behaviorally consequential, with graded betrayal as the main benchmark and Stag Hunt betrayal as a follow-up sensitivity check. The mechanism occupies a distinct computational niche from both outside-in empathy and cognitive theory of mind: inside-out metacognitive monitoring of one's own social inference channel. The Cogs vs.\ Clips transfer is best read as an architectural proof-of-concept: the same beta update rule can track teammate positional predictability in a qualitatively different domain. Affect encodes partner-specific model fitness that, in these tasks, is not well captured by reward history alone and is not recovered by the tested non-affective depth manipulations.
\end{abstract}
...
The conceptual model has three levels operating on different timescales, but the implemented core experiments primarily perform explicit inference over partner type and maintain the affective variable as an auxiliary state:

\textbf{Level 1---Observations / task context} (per-trial). Trial-by-trial interaction outcomes: active partner identity, partner's action, own payoff. In the main trust-game tasks, the current partner/context is observed rather than inferred. Updated every round.

\textbf{Level 2---Cognitive social model} (parameter learning). Latent states encoding each partner's type/strategy (cooperator, reciprocator, exploiter, random). This is the primary hidden-state inference target in the implemented experiments. Dirichlet parameters accumulate evidence over many rounds.

\textbf{Level 3---Per-partner affective state} (metacognitive integration). A slow state $\beta_k$ for each partner $k$ summarizing the running quality of the agent's social model. In the main pipeline this is updated outside the core partner-type POMDP inference loop and used to reweight policy evaluation, rather than treated as a first-class hidden-state factor everywhere. Updated by an exponentially-weighted moving average of prediction error magnitudes, resistant to single-trial fluctuations.
...
\end{equation}
For the core trust-game experiments, the implemented hidden-state structure primarily covers trial state/context and partner type. The active partner is observed from the task setup, and partner-type beliefs are updated analytically by Bayes rule. The affective variable $\beta_k$ is maintained alongside this inference process as an auxiliary metacognitive state used later in policy evaluation. A supported alternative implementation (Condition 12) represents beta with a discrete variational posterior, but this still functions as an auxiliary affective state rather than a fully symmetric hidden-state factor in the main generative model used for the core experiments.

State inference exploits the analytical solution to variational free energy minimization: for a categorical $q(s)$, the VFE-minimizing posterior equals the Bayesian posterior $q^*(s) \propto P(o \mid s) P(s)$. In the main tasks this analytical update is applied to partner-type beliefs using the likelihood matrix $\mathbf{A}$ and transition matrix $\mathbf{B}$.
...
The affective state $\beta_k$ for partner $k$ is updated from an affective charge derived from prediction error:
\begin{equation}
...
Condition 12 provides a supported alternative affective-state implementation. It casts $\beta_k$ as a discrete latent state with $L = 5$ precision levels, explicit likelihood $P(\epsilon \mid \beta) \propto \exp(\alpha(\sigma_0^2 - \epsilon^2) \cdot \beta)$, and tridiagonal persistent transition dynamics. The posterior update uses the same categorical Bayesian machinery as partner-type inference:
\begin{equation}
  q(\beta_k^{(t)}) \propto P(\epsilon_k^{(t)} \mid \beta_k) \sum_{\beta_k^{(t-1)}} P(\beta_k^{(t)} \mid \beta_k^{(t-1)})\, q(\beta_k^{(t-1)})
\end{equation}
In stable conditions, continuous EMA and discrete formulations are indistinguishable ($d = 0.001$). Under betrayal, the discrete formulation underperforms moderately ($d = 0.41$) due to the transition matrix constraining single-step posterior shifts---a feature reflecting a stronger prior on precision stability. Even in this variational form, beta remains an auxiliary affective state for the core experiments rather than a first-class hidden-state factor across the whole planning pipeline.
...
We test three 2$\times$2 symmetric games with zero code changes---only the payoff matrix differs (Table~\ref{tab:payoffs}):
...
The central result is task-bounded: under sophisticated inference in the current binary trust game, the tested non-affective depth sweep is flat, while affect provides consistent augmentation (Table~\ref{tab:horizon}).
...
\bottomrule
\end{tabular}
\caption{Horizon sweep results (100 replications $\times$ 200 rounds). Non-affective planners are flat across depths ($p > 0.93$ for all pairwise comparisons). Affect adds $\sim\!46$ payoff points at every depth. C2 vs C8: $p = 0.94$ (indistinguishable). C8 mean is rounded; the raw per-seed results were not archived, but the C2 vs C8 $p$-value confirms statistical equivalence.}
\label{tab:horizon}
\end{table}

The non-affective depth curve is flat: every pairwise comparison among C1, C4, C6, C7 has $p > 0.93$. Adding affect at depth $\tau = 2$ (C2) or $\tau = 8$ (C8) produces $\sim\!46$-point improvement, confirming that the augmentation is orthogonal to the tested depth manipulation in this task. This does not imply that depth is universally irrelevant. Rather, in the current binary-action setting, deeper non-affective rollout does not change outcomes, plausibly because the transition structure is effectively action-independent over the tested horizon and the binary policy distribution is already near saturation. The affect $\times$ depth interaction (Table~\ref{tab:orthogonality}) therefore supports an additive interpretation within this task family.
...
This comparison is task-dependent (Table~\ref{tab:h3}):
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.85\textwidth]{figures/beta_trajectory.pdf}
\caption{Beta trajectory for the betraying partner (partner 0) under the scheduled cooperator$\to$exploiter switch at round 31. The affective state drops sharply after the switch as prediction errors increase, then stabilizes at a low-precision regime reflecting the agent's updated assessment of model reliability. Shaded region: $\pm$1\,SE across seeds.}
\label{fig:beta_trajectory}
\end{figure}

\subsection{Predictive Score Comparison}

Each agent condition (Table~\ref{tab:conditions}) is treated as a competing predictive model, with per-round predictive log-likelihood $\ell_t = \log p(o_t \mid m, h_{<t})$ accumulated over all rounds. We compare cumulative predictive log-scores across seeds. Random-effects BMS \citep{stephan2009bayesian} with protected exceedance probabilities \citep{rigoux2014bayesian} is then applied to these per-seed predictive scores as a group-level summary. This should be read as predictive-score comparison, not as exact marginal model evidence.
...
\cmidrule(lr){2-3}
Setting & Winner & Exceedance & C2 vs C5 $\Delta \log_{10}$ score \\
\midrule
Default (50 seeds) & C5 & 1.000 & $-0.12$ (negligible) \\
Betrayal (50 seeds) & C2 & 0.998 & $+2.70$ (large) \\
\bottomrule
\end{tabular}
\caption{Predictive score comparison. Under betrayal, C2 attains the best cumulative predictive score, and RFX-BMS over per-seed predictive scores assigns it the highest exceedance probability. Under default conditions, C5 slightly edges C2.}
\label{tab:bms}
\end{table}

The betrayal result (Table~\ref{tab:bms}) is particularly strong in predictive terms: precision tracking produces the best cumulative predictive score under volatility, not just higher payoffs. The cumulative predictive log-score difference against C1 is about $+3.0$ on a $\log_{10}$ scale, and RFX-BMS over per-seed predictive scores assigns high exceedance probability to C2 in betrayal.

\subsection{Cross-Game Generalization}

Affect augmentation generalizes across game types, with game-dependent patterns (Table~\ref{tab:crossgame}):
...
The graded game confirms that the precision channel was structurally inert in binary games (policy entropy $< 0.01$) and activates it (Table~\ref{tab:graded}; $H_{q_\pi} \approx 5.8$):
...
\subsection{Precision Modulation Pathway Validation}

To test whether beta can improve decisions through direct precision scaling (not just terminal value weighting), we implement and test the precision modulation pathway: $\gamma_k = \gamma(1 + \beta_k)$, which scales the policy precision for each partner interaction. This pathway is structurally inert when $\gamma$ is constant, but activates when beta varies sufficiently to shift softmax sharpness.
...
\subsubsection{Graded Betrayal: Primary Clinical Sensitivity Benchmark}

Combining the graded game with betrayal stress produces massive between-clinical differentiation (Table~\ref{tab:clinical}):
...
\bottomrule
\end{tabular}
\caption{Clinical sensitivity in graded betrayal (50 seeds $\times$ 120 rounds). $d = (\text{phenotype} - \text{reference})/\text{SD}$; positive $d$ = phenotype outperforms reference. Between-clinical spread: 80.5 points (2700$\times$ improvement over graded default).}
\label{tab:clinical}
\end{table}

Three clinically interpretable findings emerge in this primary clinical benchmark:
...
\subsubsection{Stag Hunt Betrayal: Follow-up Sensitivity Check}

The graded betrayal environment is the main clinical benchmark. The Stag Hunt betrayal environment is a follow-up sensitivity check with a different cost structure: its severe miscoordination cost amplifies the behavioral consequences of precision volatility and helps test whether the phenotype ordering is robust to a coordination-heavy task.

\begin{table}[ht]
\centering\small
\begin{tabular}{lcccc}
\toprule
Phenotype & Mean payoff & vs C2 $d$ & $p$ & $\Delta \log_{10}$ score \\
\midrule
Healthy (C2) & 489.7 & --- & --- & --- \\
Alexithymia (C9) & 497.1 & $+0.20$ & 0.318 & $+0.12$ (small advantage) \\
Depression (C11) & 484.6 & $-0.13$ & 0.510 & $-0.66$ (small disadvantage) \\
Borderline (C10) & 439.8 & $-0.72$ & $<$0.001 & $-2.89$ (largest disadvantage) \\
No-affect (C4) & 402.7 & --- & --- & --- \\
\bottomrule
\end{tabular}
\caption{Clinical sensitivity in Stag Hunt betrayal (50 seeds $\times$ 120 rounds). $d = (\text{phenotype} - \text{reference})/\text{SD}$; positive $d$ = phenotype outperforms reference. This environment serves as a follow-up coordination-sensitive check rather than the primary clinical benchmark.}
\label{tab:clinical_sh}
\end{table}

Three findings distinguish the Stag Hunt from the graded-betrayal benchmark. First, \textbf{borderline is clearly impaired} ($d = -0.72$, $\Delta \log_{10}$ score $= -2.89$). The volatile $\beta$ dynamics ($\sigma = 0.185$, range 0.928) create frequent miscoordination at the severe sucker payoff. Borderline remains above no-affect ($d = 0.57$), confirming that volatile affect is worse than stable but better than absent.

Second, \textbf{alexithymia is indistinguishable from healthy} ($d = +0.20$, small positive predictive-score difference). Frozen precision ($\sigma = 0.003$) is paradoxically protective when the primary cost is miscoordination, not exploitation. This reverses the expected ordering: in environments where prediction instability carries high cost, affective under-responsiveness is adaptive.

Third, \textbf{depression shows a small predictive-score disadvantage but not a frequentist effect} ($\Delta \log_{10}$ score $= -0.66$, $p = 0.51$). The pessimistic $\beta_0$ is corrected by inference, consistent with the initial-condition versus dynamical-parameter distinction from the graded game.

The environment-dependent clinical ordering---alexithymia protective in Stag Hunt and advantageous in graded betrayal, borderline most impaired under high miscoordination cost---demonstrates that computational impairments are not intrinsic to the phenotype but emerge from phenotype--environment interaction.

\begin{figure}[ht]
\centering
\includegraphics[width=0.85\textwidth]{figures/clinical_beta.pdf}
\caption{Beta trajectories for the betraying partner across clinical phenotypes under the PD betrayal protocol. Borderline (C10) shows high-amplitude oscillations that compound into progressive deterioration; alexithymia (C9) shows blunted response, paradoxically protective under acute volatility; depression (C11) starts low but self-corrects via evidence accumulation, converging with healthy (C2) within $\sim$30 rounds. Shaded regions: $\pm$1\,SE.}
\label{fig:clinical_beta}
\end{figure}

\subsection{Discrete Variational Beta Validation}

The discrete Bayesian beta formulation (C12, $L = 5$ levels) is behaviorally equivalent to the continuous EMA (C2) in stable environments ($d = 0.001$, $p = 0.99$) and both outperform the no-affect baseline ($d \approx 0.6$, $p = 0.003$). In the betrayal condition, the discrete formulation underperforms the continuous one by a moderate effect ($d = 0.41$, $p = 0.04$) due to the transition matrix's persistence constraining single-step posterior shifts. Both still outperform the baseline. The divergence reflects a difference in the prior on precision volatility, not a different role for beta: Condition 12 remains an alternative auxiliary affective-state implementation, not a redefinition of the core hidden-state structure.

\subsection{Spatial Multi-Agent Transfer: Cogs vs.\ Clips}
\label{sec:cvc}

To test whether the precision-tracking architecture transfers beyond matrix games, we deploy it in a qualitatively different domain: CoGames/Cogs vs.\ Clips (CvC), a real-time spatial multi-agent benchmark on a $98 \times 98$ gridworld (``machina\_1'' mission, 8 agents, 1000 steps). Agents observe a $13 \times 13$ egocentric diamond of the environment and choose from 5 actions (noop + 4 cardinal moves). The task requires coordinated resource gathering: mine ore from extractors, deposit at hubs for hearts, then align junctions for team reward. Unlike the trust game's discrete partner interactions, CvC demands continuous spatial navigation, multi-step planning, and implicit coordination with teammates whose behavior is observed only through local position tracking.

\paragraph{Architecture.} We implement two policies. \emph{ScoringLoopPolicy} is a state-machine baseline that cycles through GET\_GEAR $\to$ MINE\_ORE $\to$ DEPOSIT $\to$ ALIGN\_JUNCTION using BFS pathfinding with movement-failure wall learning. \emph{AffectCvCPolicy} extends the baseline with per-teammate precision tracking: for each observed teammate $k$, the agent predicts their next position from previous velocity and updates $\beta_k$ via the same EMA surprise rule (Eq.~\ref{eq:beta_update}), with positional prediction error replacing cooperation prediction error. The team-average $\bar{\beta}$ modulates coordination strategy: high precision triggers cooperative thresholds (earlier resource deposition, direct junction seeking), while low precision triggers independent strategies (resource stockpiling, self-sufficient operation).

\paragraph{Results.} Both our policies achieve non-zero reward on the CvC benchmark (Table~\ref{tab:cvc}), substantially outperforming the CoGames built-in StarterPolicy, which scores zero reward and zero aligned junctions due to navigation failures (7,311 stuck steps out of 8,000 agent-steps). BFS pathfinding with aoe\_mask-based wall detection achieves 84--91\% movement success. The beta-tracking mechanism correctly identifies all 7 teammates via their \texttt{agent\_id} observation feature and maintains per-partner precision estimates.

\begin{table}[ht]
\centering
\caption{CvC benchmark results (machina\_1, 1000 steps, 10 seeds). Reward is per-agent mean $\pm$ SD.}
\label{tab:cvc}
\begin{tabular}{lcccc}
\toprule
Policy & Mean Reward & Aligned Junctions & Hearts & Max Stuck Steps \\
\midrule
ScoringLoopPolicy   & $0.072 \pm 0.030$ & $2.5 \pm 2.1$ & 6.3 & 183 \\
AffectCvCPolicy     & $0.071 \pm 0.032$ & $1.6 \pm 0.7$ & 5.9 &  61 \\
StarterPolicy (CG)  & $0.000 \pm 0.000$ & $0.0 \pm 0.0$ & 7.0 & 7311 \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Interpretation.} In homogeneous teams (all agents run the same policy), $\bar{\beta}$ stabilizes at $\sim$0.65---consistent with high teammate reliability. With team precision consistently above the cooperation threshold, the affect mechanism produces similar mean reward to the baseline ($0.071$ vs.\ $0.072$). The affect policy shows lower navigation failures (max 61 stuck steps vs.\ 183 for baseline), suggesting the teammate-tracking computation may modestly improve pathfinding robustness by providing additional spatial awareness. Behavioral differentiation in mean reward requires heterogeneous teams or partner volatility, paralleling the trust-game finding that affect augmentation is most pronounced under betrayal stress. The spatial domain also dampens beta dynamics relative to the trust game: positional prediction errors are normalized over a larger observation range (Manhattan distance up to 26 vs.\ binary cooperation), and random spatial movement has lower surprise than random cooperation decisions.

The key result is architectural rather than strongly validating: the per-partner precision-tracking mechanism can be instantiated in a qualitatively different domain (continuous spatial coordination vs.\ discrete economic games) without modification to the core beta update rule. The same surprise $\to$ charge $\to$ sigmoid $\to$ EMA pipeline that tracks cooperation reliability in the trust game can also track positional predictability in the gridworld, making CvC a proof-of-concept sanity check for transfer rather than strong evidence of external validity.
...
\begin{enumerate}[nosep]
  \item In the current binary trust game, the non-affective depth curve is flat ($\tau = 2$ to $\tau = 8$, $p > 0.93$), yet affect adds $\sim\!46$ points at every depth---ruling out the interpretation that affect is merely approximating the tested deeper search settings.
  ...
  \item C8 (deep + affect) $\approx$ C2 (shallow + affect), $p = 0.94$---confirming the augmentation is additive with respect to the tested depth manipulation.
\end{enumerate}

The old framing---``affect compensates for shallow planning depth''---is replaced by a narrower claim: in these tasks, affect encodes partner-specific model fitness (Eq.~\ref{eq:precision_weight}), a second-order signal (reliability of predictions) that average reward does not preserve and that the tested non-affective depth sweep does not recover. This is why both precision tracking (C2) and reward averaging (C5) outperform the non-affective conditions: both provide a terminal signal beyond raw EFE, but they summarize different aspects of interaction history.
...
\begin{itemize}[nosep]
  \item \textbf{Precision tracking excels when model fitness and reward diverge}---betrayal scenarios where attractive reward history masks predictive failure. Under binary betrayal, precision tracking wins ($d = 0.59$); under PD and Stag Hunt betrayal, it also attains the strongest predictive scores.
  ...
  \item \textbf{Game structure mediates the competition.} The Stag Hunt favors precision tracking in both default and betrayal, because its severe miscoordination penalty makes prediction accuracy critical. Chicken favors reward averaging because its reward gradient is more informative than prediction confidence.
\end{itemize}
...
\begin{itemize}[nosep]
  ...
  \item The trust-game action space remains small (2 or 24 actions). The CvC spatial domain (Section~\ref{sec:cvc}) demonstrates architectural transfer to continuous navigation, but the trust-game beta parameters produce overly stable precision in the spatial domain, suggesting domain-specific calibration is needed for behavioral modulation beyond architectural proof-of-concept.
...
The primary experiments use terminal-value weighting ($\mu$-scaled $\beta_k$). The precision modulation pathway ($\gamma_k = \gamma(1 + \beta_k)$) was validated in the graded betrayal environment (Section~\ref{sec:results}), confirming the mechanism reduces $q_\pi$ entropy by 0.44 nats with a directionally positive but non-significant payoff effect ($d = 0.21$, $n = 50$). Future work should test this pathway with larger samples, across more environments, and in interaction with the $\mu$-pathway to determine whether the two channels provide complementary or redundant contributions.

% ══════════════════════════════════════════════════════════════════════════════
\section{Conclusion}
\label{sec:conclusion}

We have demonstrated that per-partner metacognitive precision tracking provides orthogonal augmentation to explicit planning in multi-agent active inference. In the implemented core experiments, the mechanism augments partner-type inference with an auxiliary affective precision-like state that carries partner-specific model-fitness information. Within the current binary task, this information is not recovered by the tested non-affective depth sweep. The computational lesion reproduces the somatic marker deficit in a social context, the mechanism generalizes across game types, and the CvC transfer offers an architectural proof-of-concept in a qualitatively different spatial multi-agent domain. Precision tracking and reward averaging occupy complementary, context-dependent informational niches: precision tracking excels under volatility and miscoordination, while reward averaging dominates in continuous-gradient settings. Predictive log-score comparisons favor the affective model under partner volatility.

The broader implication is that biological agents may have evolved per-partner affective states not merely as shortcuts for planning, but as carriers of second-order information---model reliability---that first-order quantities such as reward history do not reconstruct and that explicit planning depth may fail to recover in some task structures. Affect, in this view, is a precision signal that can occupy a specific computational niche in social cognition.

% ══════════════════════════════════════════════════════════════════════════════
% ── Implementation Note ───────────────────────────────────────────────────────
\section*{Code Availability}

The model is implemented in Python using JAX for numerical computation. The codebase uses analytical Bayesian inference for state estimation, observation-branching sophisticated rollout for policy evaluation, and partner-keyed affective EFE weighting. Experiments are parallelized at replication granularity using \texttt{ProcessPoolExecutor}. All conditions (100 replications $\times$ 200 rounds) complete in under 30 minutes on a single machine (AMD EPYC, 16 cores). Source code is available at the project repository.
