...
For narrative guardrails, claims, and planning docs, start at
...
- `scripts/analysis/make_paper_figures.py`: regenerates composite figures from
  `source_tables/` (see `docs/paper/notes/figures_tables.md`).

## Evidence Contract

Use `docs/results/current.md`, `results_digest.md`, and the paper notes in
`docs/paper/notes/` as the diagnostic evidence surface. H5 and Exp A–D use the
...
Incoming `.tex` edits (not yet required): explicit partner-implementation
paragraph and fuller POMDP Methods subsection.

For writing guardrails when editing `.tex`, use
`docs/paper/prompts/writing_model_prompt.md`.
