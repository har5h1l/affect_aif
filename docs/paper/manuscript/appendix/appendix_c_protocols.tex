...
\textbf{Precision-dynamics perturbations.} Phenotype-inspired computational variants (low-gain, high-gain, and cautious-prior profiles) perturb $\beta_k$ amplitude and stability in the graded decision regime without claiming clinical validity (Appendix~\ref{app:extended_results}).
...
H6 perturbation dynamics & graded perturbations & 200 & 30 (confirm.) / 3 (explor. diag.) \\
...
The supplementary repository contains the simulation environments, affect-runtime variants, experiment configuration files, analysis scripts, and figure-generation code used for the reported experiments. During review, this repository is anonymized; after acceptance, it will be replaced with the public archival repository.
...
\subsection{Confirmation status}

The June 6 H1 confirmation reached structural finality and supplies the
main-text model-fitness values in Section~\ref{sec:results}.

\begin{itemize}
\item \textbf{H1 model-fitness confirmation}: the final 30-seed readout uses active-encounter partial correlations to compare precision--surprise against precision--payoff associations, with shared $\beta$ included as the relational-specificity ablation.
\end{itemize}
