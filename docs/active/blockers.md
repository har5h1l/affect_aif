...
- Before making the repository public, run one final internal-doc cleanup pass.
  Leave these files in place during active development, but review or remove
  them immediately before release:
  - `docs/superpowers/plans/`
  - `docs/superpowers/specs/`
  - internal-only `docs/active/` planning notes that duplicate current public
    routes
  - internal-only `docs/results/cleanup/` manifests that describe local/server
    cleanup operations rather than reproducible public evidence
  - subtree `AGENTS.md` files if their routing guidance is no longer useful to
    external readers
